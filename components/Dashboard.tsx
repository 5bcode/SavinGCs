...
export default function Dashboard({
  ...
  // Memoize derived pot calculations to avoid running array loops and expensive string comparisons on every render
  const {
    sortedPots,
    totalSavings,
    savingsForProgress,
    ...
    // Performance Optimization: Use localeCompare for ISO date strings instead of creating Date objects
    ...
    const tSavings = pots.reduce(
      (sum: number, pot: any) => sum + (pot.total_balance || 0),
      0,
    );

    // For progress, exclude 'Unallocated' pot
    const progressPots = pots.filter((p: any) => p.name !== "Unallocated");
    const sForProgress = progressPots.reduce(
      (sum: number, pot: any) => sum + (pot.total_balance || 0),
      0,
    );
    const tGoal = progressPots.reduce(
      (sum: number, pot: any) => sum + (pot.goal_amount || 0),
      0,
    );

    // Net worth uses totalSavings. Progress uses filtered values.
    const oProgress = tGoal > 0 ? (sForProgress / tGoal) * 100 : 0;

    // Filter out Unallocated pot if balance is 0, using the sorted list
    const vPots = sorted.filter(
    ...
    return {
      ...
      savingsForProgress: sForProgress,
      totalGoal: tGoal,
