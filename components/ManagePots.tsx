                                ...
                                {!isUnallocated && (
                                    <div style={{ display: 'flex', gap: 'var(--sp-xs)' }}>
                                        <button
                                            ...
                                            title={isEditing ? `Cancel editing ${pot.name}` : `Edit ${pot.name}`}
                                            aria-label={isEditing ? `Cancel editing ${pot.name}` : `Edit ${pot.name}`}
                                        ...
                                        <button onClick={(e) => { e.stopPropagation(); handleDelete(pot.id); }} className="icon-btn" style={{ color: 'var(--error)' }} aria-label={`Delete ${pot.name}`}>
                                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                        ...
                                        <div className="stack gap-sm mb-sm">
                                            {editData.subGoals.map((sg, idx) => (
                                                <div key={idx} className="flex gap-sm items-center">
                                                    ...
                                                    />
                                                    <button type="button" className="icon-btn" aria-label={`Remove subgoal ${sg.name}`} onClick={() => {
                                                        const newSubGoals = [...editData.subGoals];
