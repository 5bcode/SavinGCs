...
export default function ManagePots({ onUpdate }: ManagePotsProps) {
      ...
      const res = await fetch("/api/pots", {
        ...
        body: JSON.stringify({
          ...
          goalAmount: formData.goalAmount
            ? parseFloat(formData.goalAmount.replace(/,/g, ""))
            : null,
  ...
  const startEditing = (pot: SavingsPot) => {
    ...
    setEditData({
      name: pot.name,
      goalAmount: pot.goal_amount
        ? pot.goal_amount.toLocaleString("en-GB", { maximumFractionDigits: 2 })
        : "",
      ...
      subGoals: potSubGoals.map((sg) => ({
        id: sg.id,
        name: sg.name,
        targetAmount: sg.target_amount.toLocaleString("en-GB", {
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`/api/pots/${editingPotId}`, {
        ...
        body: JSON.stringify({
          ...
          goalAmount: editData.goalAmount
            ? parseFloat(editData.goalAmount.replace(/,/g, ""))
            : null,
      ...
      // Identify deletions
      const toDelete = originalSubGoals.filter(
        (osg) => !newSubGoals.find((nsg) => nsg.id === osg.id),
      );
      ...
      // Identify updates
      const toUpdate = newSubGoals.filter(
        (nsg) => nsg.id && originalSubGoals.find((osg) => osg.id === nsg.id),
      );
        ...
        <form onSubmit={handleSubmit} className="card mb-lg">
          ...
          <div className="form-group">
            ...
            <input
              ...
              onFocus={() =>
                stripCommasOnFocus(formData.goalAmount, (v) =>
                  setFormData({ ...formData, goalAmount: v }),
                )
              }
              onBlur={() =>
                formatAmountOnBlur(formData.goalAmount, (v) =>
                  setFormData({ ...formData, goalAmount: v }),
                )
              }
          ...
          return (
            <div key={pot.id}>
              ...
              >
                <div
                  className="pot-icon"
                  style={{ background: `${pot.color}30`, fontSize: "1.3rem" }}
                >
                ...
                {!isUnallocated && (
                  <div style={{ display: "flex", gap: "var(--sp-xs)" }}>
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        isEditing ? cancelEditing() : startEditing(pot);
                      }}
                      className="icon-btn"
                  ...
                  >
                    <div className="form-group">
                      ...
                      <input
                        ...
                        onFocus={() =>
                          stripCommasOnFocus(editData.goalAmount, (v) =>
                            setEditData({ ...editData, goalAmount: v }),
                          )
                        }
                        onBlur={() =>
                          formatAmountOnBlur(editData.goalAmount, (v) =>
                            setEditData({ ...editData, goalAmount: v }),
                          )
                        }
                    ...
                    <div className="form-group">
                      ...
                      <input
                        type="date"
                        className="form-input"
                        value={editData.goalDate}
                        onChange={(e) =>
                          setEditData({ ...editData, goalDate: e.target.value })
                        }
                        style={{ colorScheme: "dark" }}
                      />
                    </div>
                  ...
                  <div className="form-group">
                      ...
                      {editData.subGoals.map((sg, idx) => (
                        <div key={idx} className="flex gap-sm items-center">
                          ...
                          <input
                            type="text"
                            className="form-input"
                            value={sg.targetAmount} // stored as formatted string
                            aria-label="Sub-goal target amount"
                            ...
                            onChange={(e) => {
                              ...
                              setEditData({
                                ...editData,
                                subGoals: newSubGoals,
                                goalAmount: total.toLocaleString("en-GB", {
                                  maximumFractionDigits: 2,
                                }),
                              });
                            }}
                            onBlur={() => {
                              const newSubGoals = [...editData.subGoals];
                              const raw = newSubGoals[idx].targetAmount.replace(
                                /,/g,
                                "",
                              );
                              const val = parseFloat(raw);
                              if (!isNaN(val)) {
                                newSubGoals[idx].targetAmount =
                          ...
                          <button
                            type="button"
                            className="icon-btn"
                            aria-label={`Remove sub-goal ${sg.name}`}
                            onClick={() => {
                              const newSubGoals = [...editData.subGoals];
                    ...
                    <div className="flex gap-sm items-center">
                      <input
                        ...
                        onKeyDown={(e) => {
                          if (e.key === "Enter") {
                            e.preventDefault();
                            document
                              .getElementById(`new-sg-amt-${editingPotId}`)
                              ?.focus();
                          }
                        }}
                      />
                      <input
                        ...
                        aria-label="New sub-goal amount"
                        onKeyDown={(e) => {
                          if (e.key === "Enter") {
                            e.preventDefault();
                            const nameInput = document.getElementById(
                              `new-sg-name-${editingPotId}`,
                            ) as HTMLInputElement;
                            const amtInput = document.getElementById(
                              `new-sg-amt-${editingPotId}`,
                            ) as HTMLInputElement;
                            if (nameInput.value && amtInput.value) {
                              const amt = parseFloat(
                                amtInput.value.replace(/,/g, ""),
                              );
                              if (!isNaN(amt)) {
                                setEditData({
                                  ...editData,
                                  subGoals: [
                                    ...editData.subGoals,
                                    {
                                      ...
                                      targetAmount: amt.toLocaleString(
                                        "en-GB",
                                        { maximumFractionDigits: 2 },
                                      ),
                                    },
                                  ...
                                  goalAmount: (
                                    editData.subGoals.reduce(
                                      (sum, sg) =>
                                        sum +
                                        parseFloat(
                                          sg.targetAmount.replace(/,/g, ""),
                                        ),
                                      0,
                                    ) + amt
                          ...
                          if (nameInput.value && amtInput.value) {
                            ...
                            if (!isNaN(amt)) {
                              ...
                              const newSubGoals = [
                                ...
                                {
                                  name: nameInput.value,
                                  targetAmount: amt.toLocaleString("en-GB", {
                                    maximumFractionDigits: 2,
                                  }),
                                },
                              ];
